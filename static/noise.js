...
var main = function(){
    ...
    whitenoise = noiseGenerator(noise.white)
    brownnoise = noiseGenerator(noise.brown)
    pinknoise = noiseGenerator(noise.pink)
    wordFile = noiseGenerator(noise.say)
}
...
class Noise {
    ...
    say(buffer, count, multiplier=-.3){
        if(buffer==undefined) {
            return this.play(this.say, multiplier, count)
        }

        for (var i = 0; i < count; i++) {
            // Math.random() is in [0; 1.0]
            // audio needs to be in [-1.0; 1.0]
            buffer[i] = wordString[i]
        }
    }
